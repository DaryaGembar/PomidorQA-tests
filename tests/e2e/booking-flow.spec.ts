...
test('основной путь + гонка за слот: регистрация → навык → слот → поиск в каталоге → бронирование → «Мои встречи» у обоих → второй гость видит ошибку', async ({
  ...
  try {
    await test.step('Хост: регистрируется в PomidorQA (API)', async () => {
    ...
    await test.step('Гость2: регистрируется отдельным аккаунтом(API)', async () => {
    ...
    await deleteAccountViaApi(guest2Context).catch(() => undefined);
