const REVIEWED_EXACT_PATHS = new Set(['eslint.config.mjs', 'package.json', 'playwright.config.ts']);

export const REVIEW_MARKER_PREFIX = '<!-- pomidorqa-ai-review:';

function sanitizeReviewText(value) {
  // Нулевой пробел после @ ломает @mention-механику GitHub
  return value.trim().replaceAll('@', '@​');
}

// Ревьюятся любые ветки, кроме основных
export function isReviewableBranch(branch) {
  return !/^(main|master)$/i.test(branch);
}
...
export function buildReviewConclusion(comments) {
  if (!comments.length) {
    return '**Вердикт: зачёт.** CI завершился успешно. Доказуемых нарушений TEST-CODEX.md в добавленных строках текущего Pull Request не найдено.';
  }

  const needsWork = comments.some((comment) => ['P1', 'P2'].includes(comment.priority));
